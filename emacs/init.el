...
(use-package emacs
  ...
  :config
  ;; macOS keyboard layouts vs. Emacs key bindings.
  ;; Bindings are learned on the "Dev Dvorak" layout, whose ⌘ plane is
  ;; QWERTY (⌘-shortcuts sit at QWERTY positions system-wide). The NS port
  ;; ignores ⌘ planes and reports super keys from the base plane, so ⌘C
  ;; arrives as s-j; and under the Russian/Ukrainian layouts every chord
  ;; arrives as an unbound Cyrillic key (C-и). Rule: a modified key behaves
  ;; as if Dev Dvorak were active — C-/M- chords go to its base plane, ⌘
  ;; shortcuts to its ⌘ plane — so every binding stays on the same physical
  ;; key under any layout, while plain typing is untouched. The Cyrillic
  ;; layouts translate only characters Dev Dvorak has nowhere (their
  ;; letters): `key-translation-map' is global, so translating a shared key
  ;; like "/" would also hijack it while typing Dvorak.
  ;; Tables generated by:
  ;;   swift .claude/skills/dotfiles/scripts/mac-layout-table.swift "Dev Dvorak" Russian Ukrainian
  (defun my/translate-keys (from to modifier-sets)
    "Make each char of FROM act as the aligned char of TO under MODIFIER-SETS.
Adds `key-translation-map' entries; unmodified typing is untouched."
    (dolist (mods modifier-sets)
      (dotimes (i (length from))
        (define-key key-translation-map
                    (vector (event-convert-list (append mods (list (aref from i)))))
                    (vector (event-convert-list (append mods (list (aref to i)))))))))

  (when (featurep 'ns)
    (let ((chords '((control) (meta) (control meta) (control shift) (meta shift)))
          (cmd    '((super) (super shift)))
          (dv-from   "oeudi'qjkx;,.pfy&[{}=(#+`*)]@rg?clnh-tswzbv/OEUDI\"QJKX:<>PFY12346597%80^RG!CLNH_TSWZBV")
          (dv-qwerty "sdfhgzxcvbqweryt123465=97-80]ou[iplj'k;,/n.`SDFHGZXCVBQWERYT!@#$^%(&_*)}OU{IPLJ\"K:<?N>")
          (ru-from   "фыварпячсмийцукнеъщгхшздоэлжёбтьюФЫВАРПЯЧСМИЙЦУКНЕ№ЪЩГХШЗДОЭЛЖЁБТЬЮ")
          (ru-dvorak "aoeudi'qjkx;,.pfy@rg?clnh-ts\\wbmvAOEUDI\"QJKX:<>PFY3^RG!CLNH_TS|WBMV")
          (ru-qwerty "asdfhgzxcvbqweryt]ou[iplj'k;\\,nm.ASDFHGZXCVBQWERYT#}OU{IPLJ\"K:|<NM>")
          (ua-from   "фіварпячсмийцукнеїщгхшздоєлжʼбтьюґФІВАРПЯЧСМИЙЦУКНЕ№ЇЩГХШЗДОЄЛЖ₴БТЬЮҐ")
          (ua-dvorak "aoeudi'qjkx;,.pfy@rg?clnh-ts\\wbmv/AOEUDI\"QJKX:<>PFY3^RG!CLNH_TS|WBMV~")
          (ua-qwerty "asdfhgzxcvbqweryt]ou[iplj'k;\\,nm.`ASDFHGZXCVBQWERYT#}OU{IPLJ\"K:|<NM>~"))
      (my/translate-keys dv-from dv-qwerty cmd)
      (my/translate-keys ru-from ru-dvorak chords)
      (my/translate-keys ru-from ru-qwerty cmd)
      (my/translate-keys ua-from ua-dvorak chords)
      (my/translate-keys ua-from ua-qwerty cmd)))
